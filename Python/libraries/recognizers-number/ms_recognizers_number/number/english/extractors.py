...
from ms_recognizers_number.number.models import NumberMode, LongFormatMode
from ms_recognizers_number.resources import BaseNumbers
from ms_recognizers_number.resources.english_numeric import EnglishNumeric
from ms_recognizers_number.number.extractors import ReVal, ReRe, BaseNumberExtractor, BasePercentageExtractor, BaseMergedNumberExtractor
        ...
        self.__regexes = [
            ReVal(
                re=RegExpUtility.get_safe_reg_exp(
                    EnglishNumeric.NumbersWithPlaceHolder(placeholder)),
                val='IntegerNum'),
            ReVal(
                re=RegExpUtility.get_safe_reg_exp(
...
class EnglishDoubleExtractor(BaseNumberExtractor):
    ...
    def __init__(self, placeholder):
        self.__regexes = [
            ReVal(
                re=RegExpUtility.get_safe_reg_exp(
                    EnglishNumeric.DoubleDecimalPointRegex(placeholder)),
                val='DoubleNum'),
            ReVal(
                re=RegExpUtility.get_safe_reg_exp(
                    EnglishNumeric.DoubleWithoutIntegralRegex(placeholder)),
                val='DoubleNum'),
            ReVal(
                re=RegExpUtility.get_safe_reg_exp(self._generate_format_regex(
